...
- Configure IPVS on the director to forward traffic to LKE nodes using private IPs via **LVS Tunneling (-i)**.
...
Apply forwarding settings for TUN mode:
...
Render and apply the DaemonSet that configures the loopback `lo` interface with the VIP, ensures `tunl0` is up for IPIP decapsulation, and sets ARP behavior.

```bash
env LVS_VIP="$LVS_DR_IP" envsubst < configs/02-vip-share-daemonset.yaml.tpl | kubectl apply -f -

kubectl -n kube-system rollout status ds/lvs-vip-share
kubectl -n kube-system get pods -l app=lvs-vip-share -o wide
```
...
# Expected: ${LVS_DR_IP}/32 is present on lo
```

## Step 5 - Configure IPVS on director with LKE node backends

Collect LKE node private IPs from `InternalIP` addresses:

```bash
NODE_PRIVATE_IPS=$(kubectl get nodes -o json | jq -r '.items[].status.addresses[] | select(.type=="InternalIP") | .address')

echo "$NODE_PRIVATE_IPS"
# Expected: one private InternalIP per LKE node
```

Build and apply IPVS rules on director.

For LVS TUN (`-i`), the director encapsulates the packet and sends it over IPIP. NodePort is used identically on both sides (`${NODEPORT}`).
...
while IFS= read -r ip; do
  [ -z "$ip" ] && continue
  echo "ipvsadm -a -t ${LVS_DR_IP}:${NODEPORT} -r ${ip}:${NODEPORT} -i" >> "$IPVS_SCRIPT"
done <<< "$NODE_PRIVATE_IPS"
...
From director, confirm private-path backend connectivity and force source as director private IP:

```bash
while IFS= read -r ip; do
  [ -z "$ip" ] && continue
  ssh -n -i "$SSH_KEY" root@${DIRECTOR_IP} "curl -s --connect-timeout 2 --interface ${DIRECTOR_PRIVATE_IP} \"http://${ip}:${NODEPORT}/\" | head -20"
  echo "---"
done <<< "$NODE_PRIVATE_IPS"
```
...
### Known issues with Calico and LVS Tunneling
1. **Calico drops IPIP (`ipencap`) packets**: By default, Calico installs an `iptables` rule that actively drops incoming IPIP packets from any IP not part of the Calico overlay. The DaemonSet works around this by dynamically injecting `iptables -I INPUT 1 -p ipencap -j ACCEPT` on all nodes.
2. **Calico removes foreign IPs on `tunl0`**: If the VIP is configured on the `tunl0` interface, Calico's reconciler will periodically delete it because it considers it an unknown IP. The DaemonSet works around this by adding the VIP to the loopback interface (`lo`) instead. The kernel still accepts the decapsulated packets, but Calico ignores `lo`.
